#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Hermes Agent (multi-account).
...
Commands:
  ...
  setup.py --check --account <name>         # Check a specific account
  ...
  setup.py --auth-url --account <name>      # Auth URL for a specific account
  setup.py --auth-code CODE                 # Exchange auth code for token
  setup.py --auth-code --account <name> CODE# Exchange for a specific account
  setup.py --revoke                         # Revoke and delete stored token
  setup.py --accounts list                  # List all stored accounts
  setup.py --install-deps                   # Install Python dependencies only

Account naming:
  - Default: stores as google_token.json (backward-compatible)
  - Named:   stores as google_token.<name>.json
  - Account names are alphanumeric + hyphens, lowercase only.
...
def _resolve_token_path(account: str | None = None) -> Path:
    ...
    if not re.match(r"^[a-z0-9][a-z0-9_-]*$", account):
        print(f"ERROR: Invalid account name '{account}'. Use lowercase letters, digits, hyphens, underscores.")
        sys.exit(1)
    return HERMES_HOME / f"google_token.{account}.json"


def _resolve_pending_path(account: str | None = None) -> Path:
    """Return the pending-auth path for the given account."""
    account = (account or "").strip().lower()
    if not account or account == "default":
        return HERMES_HOME / "google_oauth_pending.json"
    return HERMES_HOME / f"google_oauth_pending.{account}.json"
...
def _list_accounts() -> list[dict]:
    ...
    for f in sorted(HERMES_HOME.iterdir()):
        ...
        # Try to extract email from the token payload (id_token field contains it)
        email = ""
        ...
        if isinstance(scopes, list):
            scopes = ", ".join(s.split(".")[-1] for s in scopes)
...
def check_auth_live(account: str | None = None):
    ...
    try:
        ...
        print(f"LIVE_CHECK_OK: Real API call succeeded (account={account or 'default'}).")
        return True
...
def _save_pending_auth(*, state: str, code_verifier: str, account: str | None = None):
    """Persist the OAuth session bits needed for a later token exchange."""
    pending_path = _resolve_pending_path(account)
...
def exchange_auth_code(code: str, account: str | None = None):
    ...
    print(f"Account: {acct_label}  |  Profile: {display_hermes_home()}")
...
def accounts_list():
    ...
    if not accounts:
        print(json.dumps({"accounts": [], "message": "No accounts found. Run --auth-url to add one."}, indent=2))
        return
    print(json.dumps({"accounts": accounts}, indent=2, ensure_ascii=False))


def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Hermes (multi-account)")

    parser.add_argument("--account", default="", help="Account name (default: 'default', stored as google_token.json)")
    ...
    group.add_argument("--accounts", action="store_true", help="List all stored accounts")
    ...
    if args.check:
        sys.exit(0 if check_auth(account=account) else 1)
    if getattr(args, "check_live", False):
        sys.exit(0 if check_auth_live(account=account) else 1)
    elif args.client_secret:
        store_client_secret(args.client_secret)
    elif args.auth_url:
        get_auth_url(account=account)
    elif args.auth_code:
        exchange_auth_code(args.auth_code, account=account)
    elif args.revoke:
        revoke(account=account)
    elif args.accounts:
        accounts_list()
