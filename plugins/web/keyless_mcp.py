...
def mcp_call(
    ...
    if response.status_code >= 400:
        raise KeylessMCPError(
            f"HTTP {response.status_code}: {_decode_body(response)[:300]}"
        )
    return _parse_mcp_body(_decode_body(response))


def _decode_body(response) -> str:
    """Decode a JSON-RPC/SSE body as UTF-8 (the spec default), not via
    ``response.text``.

    ``requests`` falls back to ISO-8859-1 when the content-type carries no
    charset (Exa's ``text/event-stream`` does not). A latin-1 decode turns
    the UTF-8 continuation byte 0x85 — present in common accented letters
    like Vietnamese "ễ" (e1 bb 85) or Polish "ą" (c4 85) — into U+0085
    (NEL), which ``str.splitlines()`` treats as a line break. That splits
    the single SSE ``data:`` line mid-JSON and every non-ASCII-heavy query
    fails with "Unrecognized MCP response shape". Honor an explicit
    ``charset=`` from the header when present; otherwise decode UTF-8.
    """
