...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        # Window style 0 = hidden; bWaitOnReturn True = the task stays Running
        # for the gateway's whole lifetime so Last Result carries the real exit
        # code (see rc propagation below). The console python's one console is
        # created hidden and inherited by all descendants, so nothing flashes.
        f"rc = sh.Run({_quote_vbs_string(command_line)}, 0, True)",
        # Custom patch (2026-09-06): propagate the gateway's exit code so the
        # task's Last Result reflects reality (75 = self-kill) instead of
        # always reporting 0. RestartOnFailure still does not fire on this
        # machine (verified with an isolated dummy task), so the real
        # resurrection path is scripts/gateway-watchdog.py
        # (Hermes_Gateway_Watchdog, every 5 min).
